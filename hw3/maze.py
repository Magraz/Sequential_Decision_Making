...
class Maze:
    ...
    def move_target(self, target_position=None):
        ...
        if target_position is None:
            new_x, new_y = self.move_maze(
                1, position=(current_x, current_y), noise=0.75
            )
        else:
    ...
    def _create_plot(self, rewards=None, title=""):
        ...
        rect = patches.Rectangle(
            rect_pos, 0.5, 0.5, linewidth=1, edgecolor="r", facecolor="none"
        )
        ax.add_patch(rect)
        ...
        rect_target = patches.Rectangle(
            target_origin, 0.7, 0.7, linewidth=1, edgecolor="g", facecolor="none"
        )
...
if __name__ == "__main__":
    ...
    assert np.all(
        original_target_loc == np.array((5, 1))
    ...
    tries = 0
    while (tries < 100) and (
        np.all(maze._get_target_location() == original_target_loc)
    ):
        maze.move_target()
        tries += 1
