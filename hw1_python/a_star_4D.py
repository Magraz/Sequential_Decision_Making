...
maze_1 = Maze4D.from_pgm(maze1_filename)
...
maze_2 = Maze4D.from_pgm(maze2_filename)
...
def A_Star(
    ...
    start_vel_diff = np.linalg.norm(start_node.state[2:] - maze.goal_state[2:])
    if start_vel_diff == 0.0:
        start_node.h_cost = float("inf")
    else:
        start_node.h_cost = (
            np.linalg.norm(start_node.state[:2] - maze.goal_state[:2]) / start_vel_diff
        )
    ...
    while len(open_list) > 0:
        ...
        for neighbor_idx in maze.get_neighbors(current_node.index):
            ...
            neighbor_vel_diff = np.linalg.norm(
                np.array(neighbor.state[2:]) - np.array(current_node.state[2:])
            )

            if neighbor_vel_diff == 0:
                time_cost = float("inf")
            else:
                time_cost = (
                    np.linalg.norm(
            ...
            # Compute costs
            tentative_g_cost = current_node.g_cost + time_cost
            goal_vel_diff = np.linalg.norm(
                np.array(neighbor.state[2:]) - np.array(maze.goal_state[2:])
            )
            if goal_vel_diff == 0:
                h = float("inf")
            else:
...
def A_Star_Epsilon(maze: Maze4D, epsilon: float = 10, runtime_limit: float = 1.0):
    ...
    while True:
        ...
        if start_vel_diff == 0.0:
            ...
            start_node.h_cost = (
                np.linalg.norm(start_node.state[:2] - maze.goal_state[:2])
                / start_vel_diff
            )
        start_node.f_cost = start_node.g_cost + start_node.h_cost
        ...
        while len(open_list) > 0:
            ...
            for neighbor_idx in maze.get_neighbors(current_node.index):
                ...
                if neighbor_vel_diff == 0:
                    ...
                    time_cost = (
                        np.linalg.norm(
                            np.array(neighbor.state[:2])
                            - np.array(current_node.state[:2])
                        )
                        / neighbor_vel_diff
                    )
